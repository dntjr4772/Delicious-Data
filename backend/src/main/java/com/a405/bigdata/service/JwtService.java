...
import com.a405.bigdata.ExceptionHandler.InvalidTokenException;
...
//	JWT Token을 분석해서 필요한 정보를 반환.
	...
	public Long getUserId(String tokenKey)
	{
	...
	public String getUserEmail(String tokenKey)
	{
		String token=tokenKey.substring(7);
		...
		try {
			claims = Jwts.parser().setSigningKey(signature.getBytes()).parseClaimsJws(token);
			logger.info("Email :"+claims.getBody().get("email"));
		} catch (final Exception e) {
			logger.error(e.getMessage());
			throw new InvalidTokenException("정상적인 토큰 값이 아닙니다.");
		}
